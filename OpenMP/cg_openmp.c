...
{
	double sum = 0.0;
        // Semble inutile omp_set_num_threads(nbth);

        // Semble inutile #pragma omp parallel for reduction(+:sum)
	for (int i = 0; i < n; i++)
		sum += x[i] * y[i];
	return sum;
}
...
// BUG - On ecrit dans le fichier solution
                for (int i = 0; i < n; i++)
                     fprintf(f_x, "%a\n", x[i]);
        }
	//for (int i = 0; i < n; i++)
